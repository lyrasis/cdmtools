...
module Cdmtools
  class CommandLine < Thor
    ...
    def initialize(*args)
      ...
      Cdmtools.const_set(:CONFIG,
        Cdmtools::ConfigReader.new(config: options[:config]))
    end
    ...
    no_commands {
      def get_colls
        if options[:coll].nil? || options[:coll].empty?
          ...
          coll_list = if Cdmtools::CONFIG.colls.length > 0
            Cdmtools::CONFIG.colls
          else
            []
          end
        else
        ...
        Cdmtools::CollDataParser.new(coll_list).colls
      end
    }

    class_option :config, type: "string", default: "config/config.yaml",
      aliases: "-c"
    ...
    LONGDESC
    option :coll,
      desc: "comma-separated list of collection aliases to include in processing", default: ""
    def get_field_data
      colls = get_colls
    ...
    desc "get_top_records",
      "get and process CDM records for top level objects in collections"
    long_desc <<-LONGDESC
    ...
    option :coll,
      desc: "comma-separated list of collection aliases to include in processing", default: ""
    option :force,
      desc: "boolean (true, false) - whether to force refresh of data", default: "false"
    def get_top_records
      colls = get_colls
      colls.each { |coll| coll.get_top_records(options[:force]) }
    end

    desc "get_child_records",
      "get and process CDM records for child objects in collections"
    long_desc <<-LONGDESC
    ...
    desc "report_cdm_error_records",
      "produce report of missing/error cdmrecords"
    long_desc <<~LONGDESC
    ...
    desc "delete_cdm_error_records",
      "delete missing/error cdmrecords and migrecs, based on problem record report"
    long_desc <<-LONGDESC
    ...
    def delete_cdm_error_records
      ...
      if File.file?(report_path)
        ...
        CSV.read(report_path, headers: true).each do |err|
          ...
          [cdmrec, migrec, cleanrec].each { |rec|
            File.delete(rec) if File.file?(rec)
          }
    ...
    desc "report_mig_error_records",
      "produce report of error migrecords that will not be able to be migrated"
    long_desc <<~LONGDESC
    ...
    desc "finalize_migration_records",
      "set `migfiletype` field and changes `{}` values to `''` values in `_migrecords` directory for all records in all collections"
    long_desc <<-LONGDESC
    ...
    desc "get_thumbnails",
      "downloads thumbnails for specified collection(s) or all collections"
    long_desc <<-LONGDESC
    ...
    desc "report_object_stats",
      "prints to screen the number of number and type of objects in each collection"
    long_desc <<-LONGDESC
    ...
    desc "print_object_hash",
      "prints to screen the object hash -- for debugging"
    option :coll,
      desc: "comma-separated list of collection aliases to include in processing", default: ""
    ...
    desc "report_object_file_counts",
      "prints to screen the number of object files harvested for each collection"
    long_desc <<-LONGDESC
    ...
    desc "report_object_count_totals",
      "prints to screen the total number of objects for collections"
    long_desc <<-LONGDESC
    ...
    desc "report_object_filesize_mismatches",
      "prints to screen the collection alias/pointers for harvested objects with filesize mismatches"
    long_desc <<-LONGDESC
    ...
    desc "report_cumulative_filesizes",
      "prints the cumulative filesize of specified collection(s)"
    long_desc <<-LONGDESC
