  ...
  class RecordGetter
    ...
    def initialize(coll, pointer)
      ...
      if File::exist?(@filename)
        Cdmtools::LOG.info("Record exists at #{@filename}. Will not retrieve from API and overwrite.")
      else
        @rec = get_record
        write_record if @rec
      end
    ...
    def get_record
      ...
      if result.is_a?(Net::HTTPSuccess)
        Cdmtools::LOG.info("Retrieved record for #{@coll.alias}/#{@pointer} from API")
    ...
    def write_record
      ...
      Cdmtools::LOG.info("Wrote record to #{@filename}")
    end
